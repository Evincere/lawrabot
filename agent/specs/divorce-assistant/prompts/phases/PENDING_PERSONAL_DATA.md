...
4. Cuando el usuario envíe las fotos/archivos (bloques `[MEDIA]`), llama a `submit_digital_evidence` con `documentType="DNI_FRONT"` para la primera imagen (frente) y `documentType="DNI_BACK"` para la segunda (dorso), ambas sin `taskId`.
